...
<html>
			...
			$(function() {
			    $("#btnDelete").click(function(){
			       var ids = $(".check[type=checkbox]:checked").serialize();
					...
					$.ajax({
			url: "${pageContext.request.contextPath}/admin/banjimanager.do?actiontype=delete",
			                     method: 'post',
			                     data: ids,
	...
	<div class="search-title">
		...
		<div class="description">
			<a
				href="${pageContext.request.contextPath}/admin/banjimanager.do?actiontype=load&forwardurl=/admin/banjiadd.jsp">新建班组</a>
		</div>
	</div>
	<!-- 搜索控件开始 -->
	<div class="search-options">
		<form id="searchForm"
			action="${pageContext.request.contextPath}/admin/banjimanager.do"
			method="post">
	...
	<table id="banji" width="100%" border="0" cellspacing="0"
		...
		<tbody>
			...
			<%	
							...
							%>
			<tr>
				<td>&nbsp<input id="chk<%=tembanji.getId()%>" class="check"
					name="ids" type="checkbox" value='<%=tembanji.getId()%>'/></td>
				<td><%=tembanji.getName()%></td>
				<td><a class="orange-href"
					href="${pageContext.request.contextPath}/admin/banjimanager.do?actiontype=load&id=<%=tembanji.getId()%>&forwardurl=/admin/banjiadd.jsp">修改</a>
