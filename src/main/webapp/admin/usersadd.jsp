       ...
       var initControl=function (){
        ...
        $(function (){
            ...
            $("#roleid").change(function(){
                $("[name=rolename]").val($("#roleid option:selected").text());
			});
            $("[name=rolename]").val($("#roleid option:selected").text());
					...
					<table class="grid" cellspacing="1" width="100%">
						...
						<tr>
							...
							<td>
								...
								<web:dropdownlist name="roleid" id="roleid" cssclass="dropdown"  value="${requestScope.users.roleid}" datasource="${roleid_datasource}"   textfieldname="name" valuefieldname="id">
								</web:dropdownlist>
								<input id="hidrolename" name="rolename" type="hidden" />
						...
						<tr>
							...
							<td>
								${users.username}
								<input name="creator" validate="{required:true,messages:{required:'请输入创建人'}}"  value="${users.username}"
									class="input-txt" type="hidden"  />
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input name="email" validate="{required:true,email:true,messages:{required:'请输入邮箱',email:'输入有效邮箱'}}"  value="${requestScope.users.email}"
						...
						<tr>
							...
							<td>
								<input name="tel" value="${requestScope.users.tel}"  validate="{required:true,mobile:true,messages:{required:'请输入电话号码',mobile:'输入有效电话号码'}}"
